...
export default function DriverOrderDetailScreen() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(t.common.error, t.driver.failedToLoadOrder);
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      Alert.alert(t.common.error, e?.message || t.ui.failedToAcceptOrder);
    } finally {
  ...
  const handleReject = async () => {
    if (!order) return;
    Alert.alert(t.driver.rejectOrder, t.driver.rejectOrderConfirm, [
      { text: t.common.cancel, style: "cancel" },
      {
        text: t.driver.reject,
        style: "destructive",
        onPress: async () => {
          setActionLoading(true);
          try {
            ...
            Alert.alert(
              t.driver.orderRejected,
              result.reassigned
                ? t.driver.orderReassigned
                : t.driver.orderNotReassigned,
              [{ text: t.common.ok, onPress: () => router.back() }],
            );
          } catch (e: any) {
            Alert.alert(t.common.error, e?.message || t.ui.failedToRejectOrder);
          } finally {
            setActionLoading(false);
          }
        },
  ...
  const handlePickup = async () => {
    if (!order) return;
    Alert.alert(t.driver.confirmPickup, t.driver.pickedUpQuestion, [
      { text: t.common.cancel, style: "cancel" },
      {
        text: t.driver.yesPickedUp,
        onPress: async () => {
          setActionLoading(true);
          try {
            ...
            Alert.alert(
              t.common.error,
              e?.message || t.ui.failedToMarkPickedUp,
            );
          } finally {
            setActionLoading(false);
          }
  ...
  const handleDeliver = async () => {
    if (!order) return;
    Alert.alert(t.driver.confirmDelivery, t.driver.deliveredQuestion, [
      { text: t.common.cancel, style: "cancel" },
      {
        text: t.driver.yesDelivered,
        onPress: async () => {
          setActionLoading(true);
          try {
            const updated = await driverApi.deliverOrder(order.id);
            setOrder(updated);
            Alert.alert(t.common.success, t.driver.orderDeliveredSuccess, [
              { text: t.common.ok, onPress: () => router.back() },
            ]);
          } catch (e: any) {
            Alert.alert(
              t.common.error,
              e?.message || t.ui.failedToMarkDelivered,
            );
          } finally {
            setActionLoading(false);
          }
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {order.user && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{t.driver.customer}</Text>
        ...
        {order.delivery_address && (
          <View style={styles.card}>
            <View style={styles.cardTitleRow}>
              <Text style={styles.cardTitle}>{t.driver.deliveryAddress}</Text>
              <TouchableOpacity
                style={styles.navButton}
                onPress={handleNavigate}
              >
                <Navigation size={14} color={Colors.primary900} />
                <Text style={styles.navText}>{t.driver.navigate}</Text>
              </TouchableOpacity>
            </View>
        ...
        {order.items && order.items.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              {t.driver.items} ({order.items.length})
            </Text>
            ...
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>{t.driver.total}</Text>
              <Text style={styles.totalAmount}>
                {t.common.currency} {parseFloat(order.final_amount).toFixed(2)}
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.card}>
          <View style={styles.paymentRow}>
            ...
            <Text style={styles.paymentLabel}>{t.driver.deliveryFee}</Text>
            <Text style={styles.paymentValue}>
              {t.common.currency} {parseFloat(order.delivery_fee).toFixed(2)}
            </Text>
          </View>
        </View>
      </ScrollView>
      ...
      <View style={styles.actionBar}>
        {order.status === "confirmed" && (
          <View style={styles.dualActions}>
            ...
            >
              ...
              <Text style={styles.actionText}>{t.driver.reject}</Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              {actionLoading ? (
                ...
                <>
                  ...
                  <Text style={styles.actionText}>{t.driver.accept}</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          >
            {actionLoading ? (
              ...
              <>
                ...
                <Text style={styles.actionText}>{t.driver.markAsPickedUp}</Text>
              </>
            )}
          </TouchableOpacity>
        )}
        ...
        {order.status === "delivered" && (
          <View style={[styles.deliveredBanner, styles.fullWidth]}>
            ...
            <Text style={styles.deliveredText}>{t.driver.orderDelivered}</Text>
          </View>
        )}
      </View>
    </SafeAreaView>

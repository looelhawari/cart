...
export default function DriverDashboardScreen() {
  ...
  const startLocationTracking = useCallback(async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          t.driver.permissionRequired,
          t.driver.locationPermissionNeeded,
        );
        return;
      }
  ...
  const handleToggleAvailability = async () => {
    ...
    try {
      ...
      Alert.alert(t.common.error, t.driver.failedToToggle);
    } finally {
      setToggling(false);
    }
  };
  ...
  const handleLogout = async () => {
    Alert.alert(t.alerts.logoutConfirmTitle, t.alerts.logoutConfirmMessage, [
      ...
      {
        text: t.alerts.logoutConfirmTitle,
        style: "destructive",
        onPress: async () => {
          stopLocationTracking();
          await logout();
    ...
    >
      ...
      <View style={styles.orderFooter}>
        <Text style={styles.orderAmount}>
          {t.common.currency} {parseFloat(item.final_amount).toFixed(2)}
        </Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Availability Toggle */}
      <View
        style={[styles.availabilityCard, isAvailable && styles.availableCard]}
      >
        <View>
          <Text
            style={[styles.availLabel, isAvailable && styles.availLabelActive]}
          >
            {isAvailable ? t.driver.youAreOnline : t.driver.youAreOffline}
          </Text>
          <Text style={styles.availSub}>
            {isAvailable
              ? locationEnabled
                ? t.driver.gpsTrackingActive
                : t.driver.gpsNotAvailable
              : t.driver.goOnlineToReceive}
          </Text>
        </View>
        <Switch
          value={isAvailable}
      ...
      {stats && (
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>{t.driver.orders}</Text>
          </View>
          <View style={styles.statCard}>
            <CircleCheck size={18} color="#22c55e" />
            <Text style={styles.statNum}>{stats.delivered}</Text>
            <Text style={styles.statLabel}>{t.driver.delivered}</Text>
          </View>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>{t.common.currency}</Text>
          </View>
        </View>
      )}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>{t.driver.noActiveOrders}</Text>
            <Text style={styles.emptySubtitle}>
              {isAvailable
                ? t.driver.newOrdersWillAppear
                : t.driver.goOnlineToStart}
            </Text>
          </View>
        }
      />

...
const STATUS_CONFIG = {
  ...
  expired: {
    ...
    icon: XCircle,
    labelKey: "promoCodes.expired",
  },
  scheduled: {
    color: "bg-blue-100 text-blue-700 border-blue-300",
    icon: Clock,
    labelKey: "promoCodes.scheduled",
  },
  inactive: {
  ...
  limit_reached: {
    ...
    icon: AlertCircle,
    labelKey: "promoCodes.limitReached",
  },
};
...
export default function PromoCodesPage() {
  ...
  // Managed state for product/category targeting (fixes uncontrolled checkbox bug)
  const [selectedProductIds, setSelectedProductIds] = useState<number[]>([]);
  const [selectedCategories, setSelectedCategories] = useState<
    { id: number; include_subcategories: boolean }[]
  >([]);
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreatePromoCodeData) =>
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: t("promoCodes.promoCreated"),
      });
    },
    onError: (error: any) => {
      toast({
        ...
        description:
          error?.response?.data?.message || t("promoCodes.createError"),
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t("common.success"),
        description: t("promoCodes.promoUpdated"),
      });
    },
    onError: (error: any) => {
      toast({
        title: t("common.error"),
        description:
          error?.response?.data?.message || t("promoCodes.updateError"),
        variant: "destructive",
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t("common.success"),
        description: t("promoCodes.promoDeleted"),
      });
    },
    onError: (error: any) => {
      toast({
        title: t("common.error"),
        description:
          error?.response?.data?.message || t("promoCodes.deleteError"),
        variant: "destructive",
      });
    },
  });
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: t("common.success"),
        description: t("promoCodes.promoDuplicated"),
      });
    },
    onError: (error: any) => {
      toast({
        title: t("common.error"),
        description:
          error?.response?.data?.message || t("promoCodes.duplicateError"),
        variant: "destructive",
      });
    },
  });
  ...
  const bulkStatusMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: t("common.success"),
        description: t("promoCodes.bulkUpdated", { count: data.updated }),
      });
    },
    onError: (error: any) => {
      toast({
        title: t("common.error"),
        description:
          error?.response?.data?.message || t("promoCodes.bulkUpdateError"),
        variant: "destructive",
      });
    },
  });
  ...
  const onSubmit = (data: CreatePromoCodeData) => {
    // Transform category_ids into category_data format expected by backend
    // Backend expects: category_data: [{ category_id: number, include_subcategories: boolean }]
    ...
    // Transform product_ids — ensure they are proper integers
    ...
    const cleanData = {
      ...
      usage_per_user: data.usage_per_user
        ? Number(data.usage_per_user)
        : undefined,
      minimum_spend_30days: (data as any).minimum_spend_30days
        ? Number((data as any).minimum_spend_30days)
        : undefined,
      minimum_orders_30days: (data as any).minimum_orders_30days
        ? Number((data as any).minimum_orders_30days)
        : undefined,
      specific_user_ids:
        data.specific_user_ids && typeof data.specific_user_ids === "string"
          ? (data.specific_user_ids as string)
              .split(",")
              .map((s) => Number(s.trim()))
              .filter((n) => !isNaN(n) && n > 0)
          : data.specific_user_ids,
      // Replace client-side arrays with properly formatted data
      product_ids: data.applies_to === "product" ? productIds : [],
      category_data: data.applies_to === "category" ? categoryData : [],
      // Remove category_ids since backend expects category_data
  ...
  const handleEdit = (promoCode: PromoCode) => {
    ...
    // Populate managed state for products and categories
    setSelectedProductIds(
      (promoCode as any).products?.map((p: any) => p.id) || [],
    );
    setSelectedCategories(
      (promoCode as any).categories?.map((c: any) => ({
        id: c.id,
        include_subcategories: c.pivot?.include_subcategories ?? false,
      })) || [],
    );
    ...
    reset({
      ...
      minimum_spend_30days:
        (promoCode as any).minimum_spend_30days || undefined,
      minimum_orders_30days:
        (promoCode as any).minimum_orders_30days || undefined,
      last_order_date_from:
        (promoCode as any).last_order_date_from || undefined,
      last_order_date_to: (promoCode as any).last_order_date_to || undefined,
      registration_date_from:
        (promoCode as any).registration_date_from || undefined,
      registration_date_to:
        (promoCode as any).registration_date_to || undefined,
  ...
  const copyToClipboard = (code: string) => {
    ...
    toast({
      ...
      description: t("promoCodes.codeClipboard", { code }),
    });
  };
  ...
  const handleExport = async () => {
    try {
      ...
      toast({
        title: t("common.success"),
        description: t("promoCodes.exportSuccess"),
      });
    } catch {
      toast({
        title: t("common.error"),
        description: t("promoCodes.exportError"),
        variant: "destructive",
      });
    }
  };
  ...
  const promoCodes: PromoCode[] = (() => {
    ...
    if (
      promoCodesData.data &&
      Array.isArray((promoCodesData.data as any).data)
    ) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      >
        <div>
          <h1
            className={`text-3xl font-bold flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}
          >
        ...
        <Card>
          <CardContent className="p-4">
            ...
            >
              <div>
                <p className="text-sm text-gray-500">
                  {t("promoCodes.activeCodes")}
                </p>
                <p className="text-2xl font-bold">
                  {analytics.active_codes || 0}
                </p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            >
              <div>
                <p className="text-sm text-gray-500">
                  {t("promoCodes.totalDiscounts")}
                </p>
                <p className="text-2xl font-bold">
                  {t("common.egp")}{" "}
                  {(analytics.total_discount_amount || 0).toLocaleString()}
                </p>
              </div>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="percentage">
                  {t("promoCodes.types.percentage")}
                </SelectItem>
                <SelectItem value="fixed_amount">
                  {t("promoCodes.types.fixedAmount")}
                </SelectItem>
                        ...
                        <tr className="hover:bg-gray-50">
                          ...
                          <td className="p-3">
                            <div className="flex items-center gap-2">
                              ...
                              {(promoCode as any).bogo_rules &&
                                (promoCode as any).bogo_rules.length > 0 && (
                                  <Badge
                                    variant="outline"
                          ...
                          <td className="p-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="capitalize">
                                {(promoCode as any).applies_to || "order"}
                              </span>
                            </div>
                          </td>
                          <td className="p-3">
                          ...
                          <td className="p-3 text-sm">
                            ...
                            <div className="text-gray-500">
                              {t("common.to")}{" "}
                              {format(
                                new Date(promoCode.valid_until),
                                "MMM dd, yyyy",
                              )}
                            </div>
                          </td>
                          ...
                          <td className="p-3">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  navigate(
                                    `/promo-codes/${promoCode.id}/analytics`,
                                  )
                                }
                                ...
                                <div className="grid grid-cols-4 gap-4">
                                  <div>
                                    ...
                                    <p className="font-medium">
                                      {promoCode.minimum_order
                                        ? `${t("common.egp")} ${promoCode.minimum_order}`
                                        : t("common.none")}
                                    </p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-gray-500 uppercase">
                                      {t("promoCodes.perUserLimit")}
                                    </p>
                                    <p className="font-medium">
                                      {(promoCode as any).usage_per_user ||
                                  ...
                                  <div>
                                    <p className="text-xs text-gray-500 uppercase">
                                      {t("promoCodes.productsCategories")}
                                    </p>
                                    <p className="font-medium">
                                      {(promoCode as any).products?.length || 0}{" "}
                                      {t("common.products")},{" "}
                                      {(promoCode as any).categories?.length ||
                                        0}{" "}
                                      {t("common.categories")}
                                    </p>
                                  </div>
                                    ...
                                    <div className="border-t pt-4">
                                      ...
                                      >
                                        ...
                                        <h4 className="font-semibold text-purple-900">
                                          {t("promoCodes.bogo.title")} (
                                          {(promoCode as any).bogo_rules.length}
                                          )
                                        </h4>
                                      </div>
                                      <div className="space-y-2">
                                        {(promoCode as any).bogo_rules.map(
                                          (rule: any, idx: number) => (
                                            <div
                                              key={idx}
        ...
        >
          <p className="text-sm text-gray-500">
            {t("common.showingResults", {
              from: promoCodesData.meta.from || 0,
              to: promoCodesData.meta.to || 0,
              total: promoCodesData.meta.total || 0,
            })}
          </p>
          <div className={`flex gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
            <Button
          ...
          <DialogHeader>
            ...
            >
              ...
              {editingPromoCode
                ...
                : t("promoCodes.createExtraordinary")}
            </DialogTitle>
            <DialogDescription>
            ...
            <>
                ...
                <TabsContent value="basic" className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        id="code"
                        {...register("code", {
                          required: t("promoCodes.codeRequired"),
                        })}
                        ...
                        <FormLabelWithTooltip
                          ...
                          label={
                            watchType === "percentage"
                              ? t("promoCodes.percentageValue")
                              : t("promoCodes.discountAmount")
                          }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        type="date"
                        {...register("valid_from", {
                          required: t("promoCodes.startDateRequired"),
                        })}
                      />
                      {errors.valid_from && (
                        <span className="text-xs text-red-500">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        {...register("valid_until", {
                          required: t("promoCodes.endDateRequired"),
                        })}
                      />
                      {errors.valid_until && (
                        <span className="text-xs text-red-500">
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <Label htmlFor="first_order_only">
                        {t("promoCodes.firstOrderOnly")}
                      </Label>
                    </div>
                  </div>
                </TabsContent>
                  ...
                  <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="font-semibold text-indigo-800">
                        {t("promoCodes.targeting.userSegment")}
                      </h4>
                    </div>
                    <p className="text-sm text-indigo-600 mb-4">
                      {t("promoCodes.targeting.userSegmentDesc")}
                    </p>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {/* All Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "all_users" ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-indigo-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.allUsers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.allUsersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* New Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "new_users" ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-green-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.newUsers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.newUsersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* High Spenders */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "high_spenders" ? "border-amber-500 bg-amber-50" : "border-gray-200 hover:border-amber-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.highSpenders")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.highSpendersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* Active Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "active_users" ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-blue-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.activeUsers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.activeUsersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* Delivery Lovers */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "delivery_lovers" ? "border-cyan-500 bg-cyan-50" : "border-gray-200 hover:border-cyan-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.deliveryLovers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.deliveryLoversDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* High Rated Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "high_rated" ? "border-yellow-500 bg-yellow-50" : "border-gray-200 hover:border-yellow-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.highRated")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.highRatedDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* Inactive Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "inactive_users" ? "border-gray-500 bg-gray-100" : "border-gray-200 hover:border-gray-400 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.inactiveUsers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.inactiveUsersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* Offline Users */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "offline_users" ? "border-slate-500 bg-slate-50" : "border-gray-200 hover:border-slate-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.offlineUsers")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.offlineUsersDesc")}
                              </p>
                            </div>
                          </label>
                          ...
                          {/* Custom Criteria */}
                          <label
                            className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "custom" ? "border-rose-500 bg-rose-50" : "border-gray-200 hover:border-rose-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-medium">
                                  {t("promoCodes.targeting.custom")}
                                </span>
                              </div>
                              <p className="text-xs text-gray-500 mt-1">
                                {t("promoCodes.targeting.customDesc")}
                              </p>
                            </div>
                          </label>
                        </div>
                    ...
                    {watchTargetAudience === "custom" && (
                      <div className="mt-4 p-4 bg-white rounded-lg border border-rose-200 space-y-4">
                        <h5 className="font-medium text-gray-700">
                          {t("promoCodes.targeting.customCriteria")}
                        </h5>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <FormLabelWithTooltip
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">
                              {t("promoCodes.targeting.minSpendHint")}
                            </p>
                          </div>
                          <div>
                            <FormLabelWithTooltip
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">
                              {t("promoCodes.targeting.minOrdersHint")}
                            </p>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label>
                              {t("promoCodes.targeting.lastOrderDate")}
                            </Label>
                            <div className="flex items-center gap-2">
                              <Input
                                type="date"
                                {...register("last_order_date_from" as any)}
                                placeholder={t("promoCodes.targeting.from")}
                              />
                              <Input
                                type="date"
                                {...register("last_order_date_to" as any)}
                                placeholder={t("promoCodes.targeting.to")}
                              />
                            </div>
                          </div>
                          <div>
                            <Label>
                              {t("promoCodes.targeting.registrationDate")}
                            </Label>
                            <div className="flex items-center gap-2">
                              <Input
                                type="date"
                                {...register("registration_date_from" as any)}
                                placeholder={t("promoCodes.targeting.from")}
                              />
                              <Input
                                type="date"
                                {...register("registration_date_to" as any)}
                                placeholder={t("promoCodes.targeting.to")}
                              />
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            />
                            <Input
                              {...register("specific_user_ids" as any)}
                              placeholder={t(
                                "promoCodes.targeting.specificUserIdsHint",
                              )}
                            />
                          </div>
                        </div>
                    ...
                    {watchTargetAudience === "high_spenders" && (
                      <div className="mt-4 p-4 bg-white rounded-lg border border-amber-200">
                        <div>
                          <Label>
                            {t("promoCodes.targeting.minSpendAmount")}
                          </Label>
                          <Input
                            type="number"
                            {...register("minimum_spend_30days" as any)}
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            {t("promoCodes.targeting.minSpendAmountHint")}
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="font-semibold text-emerald-800">
                        {t("promoCodes.targeting.productScope")}
                      </h4>
                    </div>
                    <p className="text-sm text-emerald-600 mb-4">
                      {t("promoCodes.targeting.productScopeDesc")}
                    </p>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <div className="grid grid-cols-3 gap-3 mb-4">
                          <label
                            className={`flex flex-col items-center justify-center p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "order" ? "border-emerald-500 bg-emerald-100" : "border-gray-200 hover:border-emerald-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            />
                            <ShoppingCart
                              className={`w-8 h-8 mb-2 ${field.value === "order" ? "text-emerald-600" : "text-gray-400"}`}
                            />
                            <span
                              className={`font-medium ${field.value === "order" ? "text-emerald-700" : "text-gray-600"}`}
                            >
                              {t("promoCodes.allProducts")}
                            </span>
                            <p className="text-xs text-gray-500 mt-1 text-center">
                              {t("promoCodes.targeting.allProductsHint")}
                            </p>
                          </label>

                          <label
                            className={`flex flex-col items-center justify-center p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "product" ? "border-emerald-500 bg-emerald-100" : "border-gray-200 hover:border-emerald-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            />
                            <Package
                              className={`w-8 h-8 mb-2 ${field.value === "product" ? "text-emerald-600" : "text-gray-400"}`}
                            />
                            <span
                              className={`font-medium ${field.value === "product" ? "text-emerald-700" : "text-gray-600"}`}
                            >
                              {t("promoCodes.specificProducts")}
                            </span>
                            <p className="text-xs text-gray-500 mt-1 text-center">
                              {t("promoCodes.targeting.specificProductsHint")}
                            </p>
                          </label>

                          <label
                            className={`flex flex-col items-center justify-center p-4 rounded-lg border-2 cursor-pointer transition-all ${field.value === "category" ? "border-emerald-500 bg-emerald-100" : "border-gray-200 hover:border-emerald-300 bg-white"}`}
                          >
                            <input
                              type="radio"
                              {...field}
                            ...
                            />
                            <Layers
                              className={`w-8 h-8 mb-2 ${field.value === "category" ? "text-emerald-600" : "text-gray-400"}`}
                            />
                            <span
                              className={`font-medium ${field.value === "category" ? "text-emerald-700" : "text-gray-600"}`}
                            >
                              {t("promoCodes.specificCategories")}
                            </span>
                            <p className="text-xs text-gray-500 mt-1 text-center">
                              {t("promoCodes.targeting.specificCategoriesHint")}
                            </p>
                          </label>
                        </div>
                      )}
                      ...
                      <div className="mt-4 bg-white rounded-lg border border-emerald-200 p-4">
                        <div className="flex items-center justify-between mb-3">
                          <Label className="text-base font-semibold">
                            {t("promoCodes.selectProductsToApply")}
                          </Label>
                          <Badge variant="outline">
                            {t("promoCodes.targeting.selectedCount", {
                        ...
                        <div className="relative mb-3">
                          ...
                          <Input
                            ...
                            placeholder={t(
                              "promoCodes.targeting.searchProductsPlaceholder",
                            )}
                        ...
                        {productsLoading ? (
                          <div
                            className={`flex items-center justify-center py-8 ${isRTL ? "flex-row-reverse" : ""}`}
                          >
                            <RefreshCw className="w-6 h-6 animate-spin text-gray-400" />
                            <span
                              className={`${isRTL ? "mr-2" : "ml-2"} text-gray-500`}
                            >
                              {t("promoCodes.loadingProducts")}
                            </span>
                          </div>
                        ) : (
                    ...
                    {watchAppliesTo === "category" && (
                      <div className="mt-4 bg-white rounded-lg border border-emerald-200 p-4">
                        <div className="flex items-center justify-between mb-3">
                          <Label className="text-base font-semibold">
                            {t("promoCodes.selectCategoriesToApply")}
                          </Label>
                          <Badge variant="outline">
                            {t("promoCodes.targeting.selectedCount", {
                        ...
                        {categoriesLoading ? (
                          ...
                          >
                            ...
                            <span
                              className={`${isRTL ? "mr-2" : "ml-2"} text-gray-500`}
                            >
                              {t("promoCodes.loadingCategories")}
                            </span>
                          </div>
                        ) : (
                          <div className="border rounded-lg max-h-60 overflow-y-auto">
                            {(categoriesData as any)?.data?.length > 0 ? (
                              (categoriesData as any).data.map(
                                (category: any) => {
                                  const isSelected = selectedCategories.some(
                                    (c) => c.id === category.id,
                                  );
                                  const catEntry = selectedCategories.find(
                                    (c) => c.id === category.id,
                                  );
                                  return (
                                    <div
                                    ...
                                    >
                                      <label
                                        className={`flex items-center gap-3 p-3 hover:bg-gray-50 cursor-pointer ${isRTL ? "flex-row-reverse" : ""}`}
                                      >
                                        <input
                                          type="checkbox"
                                          checked={isSelected}
                                        ...
                                        >
                                          <input
                                            type="checkbox"
                                            checked={
                                              catEntry?.include_subcategories ??
                                              false
                                            }
                                            onChange={(e) => {
                                              e.stopPropagation();
                                              if (isSelected) {
                                                ...
                                                // Auto-select category if toggling subcategories
                                                setSelectedCategories(
                                                  (prev) => [
                                                    ...prev,
                                                    {
                    ...
                    {watchAppliesTo === "order" && (
                      <div className="mt-4 text-center py-6 bg-white rounded-lg border border-emerald-200">
                        ...
                        <h3 className="font-semibold text-gray-700">
                          {t("promoCodes.appliesToEntireOrder")}
                        </h3>
                        <p className="text-sm text-gray-500 mt-1">
                          {t("promoCodes.entireOrderDesc")}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-orange-50 border border-pink-200 rounded-xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="font-semibold text-pink-800">
                        {t("promoCodes.targeting.promoMessage")}
                      </h4>
                    </div>
                    <p className="text-sm text-pink-600 mb-4">
                      {t("promoCodes.targeting.promoMessageDesc")}
                    </p>
                    ...
                    <div className="space-y-4">
                      <div className="bg-white rounded-lg border border-pink-200 p-4">
                        ...
                        <textarea
                          ...
                          rows={3}
                          placeholder={t(
                            "promoCodes.targeting.messageEnglishPlaceholder",
                          )}
                        />
                      </div>
                      ...
                      <div className="bg-white rounded-lg border border-pink-200 p-4">
                        ...
                        <textarea
                          ...
                          rows={3}
                          placeholder={t(
                            "promoCodes.targeting.messageArabicPlaceholder",
                          )}
                        />
                      </div>
                      ...
                      <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-xs text-orange-700">
                            {t("promoCodes.targeting.messageNote")}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {watchType !== "bogo" ? (
                    <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50">
                      ...
                      <h3 className="text-lg font-medium text-gray-700">
                        {t("promoCodes.bogo.notAvailable")}
                      </h3>
                    ...
                    <div className="space-y-4">
                      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                        <h4 className="font-medium text-purple-800 mb-2">
                          🎁 {t("promoCodes.bogo.howItWorks")}
                        </h4>
                        <ul
                          className={`text-sm text-purple-600 space-y-1 ${isRTL ? "text-right" : "text-left"}`}
                        >
                          <li>
                            • <strong>{t("promoCodes.bogo.buyFrom")}:</strong>{" "}
                            {t("promoCodes.bogo.buyFromDesc")}
                          </li>
                          <li>
                            • <strong>{t("promoCodes.bogo.get")}:</strong>{" "}
                            {t("promoCodes.bogo.getDesc")}
                          </li>
                      ...
                      {bogoFields.length === 0 ? (
                        <div className="text-center py-8 border-2 border-dashed border-purple-200 rounded-lg bg-purple-50/50">
                          ...
                          <h3 className="font-medium text-purple-700">
                            {t("promoCodes.bogo.noRules")}
                          </h3>
                          <p className="text-sm text-purple-600 mt-1">
                            {t("promoCodes.bogo.clickToAdd")}
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                            ...
                            >
                              ...
                              <div className="bg-gray-50 rounded-lg p-3 mb-3">
                                <p className="text-xs font-bold text-gray-600 mb-2">
                                  🛒 {t("promoCodes.bogo.whenCustomerBuys")}:
                                </p>
                                <div className="grid grid-cols-3 gap-3">
                                  <div>
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.buyQty",
                                      )}
                                    ...
                                    <Input
                                      ...
                                      min="1"
                                      {...register(
                                        `bogo_rules.${index}.buy_qty` as any,
                                      )}
                                      className="h-9"
                                    />
                                  </div>
                                  <div className="col-span-2">
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.buyFrom",
                                      )}
                                      labelClassName="text-xs"
                                      className="mb-0"
                                    />
                                    <Controller
                                      name={
                                        `bogo_rules.${index}.buy_scope` as any
                                      }
                                      control={control}
                                      render={({ field }) => (
                                        <Select
                                        ...
                                        >
                                          ...
                                          <SelectContent>
                                            <SelectItem value="any">
                                              {t("promoCodes.bogo.anyProduct")}
                                            </SelectItem>
                                            <SelectItem value="product">
                                              {t(
                                                "promoCodes.bogo.specificProduct",
                                              )}
                                            </SelectItem>
                                            <SelectItem value="category">
                                              {t(
                                                "promoCodes.bogo.specificCategory",
                                              )}
                                            </SelectItem>
                                          </SelectContent>
                                        </Select>
                                  ...
                                  {watch(`bogo_rules.${index}.buy_scope`) ===
                                    ...
                                    <div className="col-span-3">
                                      <FormLabelWithTooltip
                                        ...
                                        label={t(
                                          "promoCodes.bogo.selectProductBarcode",
                                        )}
                                        tooltip={t(
                                          "promoCodes.tooltips.bogo.buyProductId",
                                        )}
                                        labelClassName="text-xs"
                                        className="mb-0"
                                      />
                                      <Input
                                        type="number"
                                        placeholder={t(
                                          "promoCodes.bogo.productBarcodePlaceholder",
                                        )}
                                        {...register(
                                          `bogo_rules.${index}.buy_product_id` as any,
                                        )}
                                        className="h-9"
                                      />
                                    </div>
                                  ...
                                  {watch(`bogo_rules.${index}.buy_scope`) ===
                                    ...
                                    <div className="col-span-3">
                                      <FormLabelWithTooltip
                                        ...
                                        label={t(
                                          "promoCodes.bogo.selectCategory",
                                        )}
                                        tooltip={t(
                                          "promoCodes.tooltips.bogo.buyCategoryId",
                                        )}
                                        labelClassName="text-xs"
                                        className="mb-0"
                                      />
                                      <Controller
                                        name={
                                          `bogo_rules.${index}.buy_category_id` as any
                                        }
                                        control={control}
                                        render={({ field }) => (
                                          <Select
                                          ...
                                          >
                                            <SelectTrigger className="h-9">
                                              <SelectValue
                                                placeholder={t(
                                                  "promoCodes.bogo.selectCategoryPlaceholder",
                                                )}
                                              />
                                            </SelectTrigger>
                                            <SelectContent>
                                ...
                                <div className="grid grid-cols-4 gap-3">
                                  <div>
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.getQty",
                                      )}
                                      labelClassName="text-xs"
                                      className="mb-0"
                                    />
                                    <Input
                                      ...
                                      min="1"
                                      {...register(
                                        `bogo_rules.${index}.get_qty` as any,
                                      )}
                                      className="h-9"
                                    />
                                  </div>
                                  <div>
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.getFrom",
                                      )}
                                      labelClassName="text-xs"
                                      className="mb-0"
                                    />
                                    <Controller
                                      name={
                                        `bogo_rules.${index}.get_scope` as any
                                      }
                                      control={control}
                                      render={({ field }) => (
                                        <Select
                                  ...
                                  <div>
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.discountType",
                                      )}
                                      labelClassName="text-xs"
                                      className="mb-0"
                                    />
                                    <Controller
                                      name={
                                        `bogo_rules.${index}.get_discount_type` as any
                                      }
                                      control={control}
                                      render={({ field }) => (
                                        <Select
                                        ...
                                        >
                                          ...
                                          <SelectContent>
                                            <SelectItem value="free">
                                              {t("promoCodes.bogo.free")}
                                            </SelectItem>
                                            <SelectItem value="percentage">
                                              {t(
                                                "promoCodes.bogo.percentageOff",
                                              )}
                                            </SelectItem>
                                            <SelectItem value="fixed_amount">
                                              {t("promoCodes.bogo.fixedOff")}
                                            </SelectItem>
                                          </SelectContent>
                                        </Select>
                                      )}
                                  ...
                                  {watch(
                                    ...
                                    <div>
                                      <FormLabelWithTooltip
                                        ...
                                        label={t(
                                          "promoCodes.bogo.discountValue",
                                        )}
                                        tooltip={t(
                                          "promoCodes.tooltips.bogo.discountValue",
                                        )}
                                        labelClassName="text-xs"
                                        className="mb-0"
                                      />
                                      <Input
                                        ...
                                        min="0"
                                        {...register(
                                          `bogo_rules.${index}.get_discount_value` as any,
                                        )}
                                        placeholder={
                                          watch(
                                            `bogo_rules.${index}.get_discount_type`,
                                  ...
                                  <div>
                                    <FormLabelWithTooltip
                                      ...
                                      tooltip={t(
                                        "promoCodes.tooltips.bogo.maxPerOrder",
                                      )}
                                      labelClassName="text-xs"
                                      className="mb-0"
                                    />
                                    <Input
                                      ...
                                      min="1"
                                      {...register(
                                        `bogo_rules.${index}.max_applications_per_order` as any,
                                      )}
                                      className="h-9"
                                    />
                                  </div>
                                  {watch(`bogo_rules.${index}.get_scope`) ===
                                    ...
                                    <div className="col-span-4">
                                      <FormLabelWithTooltip
                                        ...
                                        label={t(
                                          "promoCodes.bogo.freeProductBarcode",
                                        )}
                                        tooltip={t(
                                          "promoCodes.tooltips.bogo.getProductId",
                                        )}
                                        labelClassName="text-xs"
                                        className="mb-0"
                                      />
                                      <Input
                                        ...
                                        {...register(
                                          `bogo_rules.${index}.get_product_id` as any,
                                        )}
                                        className="h-9"
                                      />
                                    </div>
                                  ...
                                  {watch(`bogo_rules.${index}.get_scope`) ===
                                    ...
                                    <div className="col-span-4">
                                      <FormLabelWithTooltip
                                        ...
                                        label={t(
                                          "promoCodes.bogo.freeFromCategory",
                                        )}
                                        tooltip={t(
                                          "promoCodes.tooltips.bogo.getCategoryId",
                                        )}
                                        labelClassName="text-xs"
                                        className="mb-0"
                                      />
                                      <Controller
                                        name={
                                          `bogo_rules.${index}.get_category_id` as any
                                        }
                                        control={control}
                                        render={({ field }) => (
                                          <Select
              ...
              <DialogFooter className="mt-6">
                ...
                >
                  ...
                  {editingPromoCode ? t("common.update") : t("common.create")}{" "}
